...
Pins (D40/I-r3): an expired `done` report is deleted WITH its chunks; an unexpired one is
untouched; a `failed` report is kept for visibility until the TTL, then deleted; a fenced loser's
orphan chunks are deleted while the winner's survive; a RUNNING report's live-attempt chunks are
never touched; chunks whose report doc vanished are deleted; a second sweep deletes nothing
...
async def test_expired_done_report_is_deleted_with_its_chunks(client) -> None:
...
async def test_stale_failed_report_is_deleted_after_the_ttl_only(client) -> None:
...
async def test_fenced_losers_orphan_chunks_are_deleted_winners_survive(client) -> None:
...
async def test_running_reports_live_attempt_is_never_touched(client) -> None:
    ...
    assert await _chunk_count(client, report_id, prior) == 0  # the stale attempt deleted


async def test_chunks_of_a_vanished_report_are_deleted(client) -> None:
...
async def test_pending_jobs_are_untouched_and_a_second_sweep_deletes_nothing(client) -> None:
    ...
    await sweep(client)
    assert await _report_exists(client, pending)  # pending never deleted

    counts = await sweep(client)  # idempotence: the second pass finds nothing OURS to delete
    assert not await _report_exists(client, done)
    assert await _chunk_count(client, done) == 0
    # our docs are gone; the second sweep deleted none of them (counts may include other tests'

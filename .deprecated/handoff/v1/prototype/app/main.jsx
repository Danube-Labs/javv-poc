...
function TimePicker({ value, onChange }) {
  ...
  const apply = (l, days) => { onChange({ label: l, days }); setOpen(false); };
  const fmtD = (iso) => {
  ...
  return (
    <div className="login-stage">
      <div className="login-card">
        ...
        <input className="fld" type="email" placeholder="you@example.com" defaultValue="lorem.ipsum@example.com" />
        ...
        <button className="btn btn-ghost btn-block" onClick={() => go("allclusters")}><Icon name="shield" size={14} />Continue with SSO</button>
        <p className="login-note">Your role (Viewer, Auditor, Operator, Security Lead, Admin) is granted by your identity-provider group.</p>
      </div>
    </div>
  );
}
...
function App() {
  ...
  switch (route.name) {
    ...
    case "images": view = firstRun ? <FirstRun go={go} clusterName={cluster.name} what="images" /> : <Images go={go} cluster={cluster} />; break;
    case "image": view = <ImageDetail go={go} image={route.data} />; break;

...
| Findings/Images/Approvals/Audit tables | PrimeVue `DataTable` **lazy mode** (server-side sort/filter/page) - the agreed default per **PLAN D27**; AG-Grid-Community is the escape-hatch only if a screen truly needs spreadsheet density |
...
- The dense-grid table engine is **decided** (PLAN D27: PrimeVue DataTable lazy, AG-Grid escape-hatch) - no longer an open question.

...
This document describes the v3-era 12-screen baseline. The **v4 deltas** - whole-app time-travel,
capability-gated UI, the full 6-state VEX triage model, the scoped risk-accept dialog, point-in-time
image view, inventory not-live states, the export dialog, and Settings → Data & OpenSearch - are
specified in **`V4-DELTA.md`** (with conflicts to reconcile against `PLAN_v4`/`SPEC_v4`/`ARCHITECTURE_v4`
flagged). Read `V4-DELTA.md` alongside this file; the prototype reflects v4.

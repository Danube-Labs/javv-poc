...
(function () {
  ...
  // ---- v4: current user + capabilities (capability-gated UI, not role names) ----
  const currentUser = {
    name: "Lorem Ipsum", initials: "LI", tone: "#C0271D", role: "Security Lead",
    caps: ["can_triage", "can_accept_audit_final", "can_export", "can_manage_users", "can_manage_retention", "can_restore_snapshot"],
  };
  const CAP_LABEL = {
    can_triage: "Triage findings",
    can_accept_audit_final: "Risk-accept (final approval)",
    can_export: "Export reports",
    can_manage_users: "Manage users & roles",
    can_manage_retention: "Manage data & retention",
    can_restore_snapshot: "Snapshot / restore",
  };
  // capability bundles per role (assigned in Admin → Users)
  const capabilityBundles = {
    Viewer: [],
    Auditor: ["can_export"],
    Operator: ["can_triage", "can_export"],
    "Security Lead": ["can_triage", "can_accept_audit_final", "can_export"],
    Admin: ["can_triage", "can_accept_audit_final", "can_export", "can_manage_users", "can_manage_retention", "can_restore_snapshot"],
  };

  // ---- v4: VEX justifications (CISA five) for not_affected ----
  const vexJustifications = [
    { id: "component_not_present", label: "Component not present", maps: "False positive", note: "The vulnerable component isn't in the image." },
    { id: "vulnerable_code_not_present", label: "Vulnerable code not present", maps: "False positive", note: "Component is present but the affected code was removed." },
    { id: "vulnerable_code_not_in_execute_path", label: "Not in execute path", maps: "Not exploitable", note: "Vulnerable code is never reached at runtime." },
    { id: "vulnerable_code_cannot_be_controlled_by_adversary", label: "Not adversary-controllable", maps: "Not exploitable", note: "An attacker can't control the inputs that reach it." },
    { id: "inline_mitigations_already_exist", label: "Inline mitigations exist", maps: "Not exploitable", note: "Compensating controls already block exploitation." },
  ];

  // ---- v4: scoped risk-accept / not-affected decisions (system-decisions) ----
  const decisions = [
    { id: "DEC-2041", cve: "CVE-2024-10042", type: "risk_accepted", scope: { namespaces: ["lorem-ipsum"], images: [] }, blastRadius: "9 images · namespace lorem-ipsum", applyBoth: true, justification: "Internal-only namespace behind mTLS; exploit needs external network. Accept until base image bump lands.", approver: "Lorem Ipsum", expiry: "Sep 15, 2026", created: "Jun 12, 2026", status: "active", cascades: true },
    { id: "DEC-2038", cve: "CVE-2024-10310", type: "not_affected", scope: { namespaces: [], images: ["dolor-svc:v1.0.0"] }, blastRadius: "1 image", applyBoth: false, scanner: "Trivy", vexJust: "vulnerable_code_not_present", justification: "Patched function stripped in our multi-stage build; Trivy matches on package metadata only.", approver: "Amet Consectetur", expiry: "—", created: "Jun 10, 2026", status: "active", cascades: false },
    { id: "DEC-2009", cve: "CVE-2024-10042", type: "risk_accepted", scope: { namespaces: ["dolor-sit-amet"], images: [] }, blastRadius: "4 images · namespace dolor-sit-amet", applyBoth: true, justification: "Superseded — re-scoped after the namespace split.", approver: "Lorem Ipsum", expiry: "Jul 1, 2026", created: "May 28, 2026", status: "revoked", revokedAt: "Jun 12, 2026", cascades: true },
    { id: "DEC-1990", cve: "CVE-2024-10042", type: "risk_accepted", scope: { namespaces: [], images: ["amet-gateway:v8.3.0"] }, blastRadius: "1 image", applyBoth: false, scanner: "Grype", justification: "30-day acceptance during the Q1 freeze.", approver: "Dolor Sit", expiry: "Mar 31, 2026", created: "Mar 1, 2026", status: "expired", cascades: false },
  ];

  // ---- v4: export jobs (run-now / scheduled off-peak) ----
  const exportJobs = [
    { id: "EXP-3120", scope: "Findings · severity ≥ HIGH", rows: 1204, mode: "offpeak", status: "ready", requested: "2h ago", ready: "1h ago", by: "Lorem Ipsum" },
    { id: "EXP-3118", scope: "Running images · all", rows: 13, mode: "now", status: "ready", requested: "3h ago", ready: "3h ago", by: "Dolor Sit" },
    { id: "EXP-3115", scope: "Findings · namespace lorem-ipsum", rows: 4187, mode: "offpeak", status: "running", requested: "10m ago", ready: "—", by: "Lorem Ipsum" },
  ];

  // ---- v4: per-digest build history for an image (point-in-time identity = digest) ----
  const digestHistory = {
    name: "lorem-api", tag: "v1.4.0",
    digests: [
      { digest: "sha256:9c2e…a1f3", from: "Jun 8, 2026", to: "now", running: true, crit: 8, high: 612, scannedRel: "5h ago", note: "current build" },
      { digest: "sha256:44ad…7b1c", from: "May 22, 2026", to: "Jun 8, 2026", running: false, crit: 11, high: 640, scannedRel: "—", note: "image build changed here" },
      { digest: "sha256:71bb…0d9e", from: "May 2, 2026", to: "May 22, 2026", running: false, crit: 14, high: 661, scannedRel: "—", note: "image build changed here" },
    ],
  };

  // ---- v4: Data & OpenSearch admin settings ----
  const dataOps = {
    retentionDays: { occurrences: 180, scanEvents: 365, auditLog: 1095, images: 90 },
    rollover: { maxDocs: "50M", maxAge: "30d", maxSize: "40gb" },
    snapshot: { repo: "s3://javv-snapshots/lorem-prod", schedule: "Daily 02:30", last: "10h ago", lastAbs: "Jun 12, 02:30", status: "ok", retained: 30 },
    staleness: { freshnessDays: 3, scannerDownDays: 7 },
  };

  // ---- v4: per-image scanner count disagreement (D5b) — attach to a few images ----
  images.forEach((im, i) => {
    if (im.scanners.length === 2) {
      const t = im.total, delta = [0, 0, 3, 0, -2, 0, 5, 0, 0, -1, 0, 0, 4][i % 13] || 0;
      im.trivyCount = t; im.grypeCount = t - delta; im.countDelta = delta;
    } else { im.trivyCount = im.total; im.grypeCount = null; im.countDelta = null; }
  });
  ...
  window.JAVV = {
    ...
    currentUser, CAP_LABEL, capabilityBundles, vexJustifications, decisions, exportJobs, digestHistory, dataOps,
  };
})();

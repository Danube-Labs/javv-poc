...
| File | What it is |
|---|---|
| [`smoke.sh`](smoke.sh) | The **level-2 e2e smoke** (risk-register #134): real Trivy + Grype as host processes scan a k3d cluster, push through the live backend into OpenSearch. Covers: auth + must-change rotation, token mint, two scan cycles (idempotency), per-scanner counts, disagreement, the **reconcile/tombstone phase** (severity-filtered cycle → present shrinks → full cycle → exact restore), scheduled jobs, log-content assertions, and (since #222) the **read/report phase**: search + cursor paging, facets, triage→journal, decision round-trip (ignore_rule projection + revoke), SLA round-trip, trends/contributors, sanitized CSV + per-scanner VEX export, M7 report enqueue, /metrics. The **M8 section** (#249) adds: point-in-time reconstruction (`as_of` at a T before a state-changing rescan returns the OLD state, and is stable across re-reads — D28), the M8c reads (provenance exact `scan_order`, images inventory, cursor-paged audit, clusters rename journal+CAS round-trip), the M8d `ptype` facet, and the M8e saved-view CRUD round-trip. The **D46 vocabulary section** (#274) proves crit→critical end-to-end: full-word `severity_canonical` on finding docs + short `crit`/`med` count columns on scan-events (the wire), the SLA-overdue regression (a near-zero `critical_days` tips a real critical overdue — the bug that hid since M5d), and full-word severities in CSV/VEX exports. Closes with a **PIT-leak-zero** check. The **negation/absence section** (#520; covers #492 · #349 §1 · #359) walks the read surface that grew after the #222/#249 phases were written, and exists for three properties a unit test cannot reach without a real corpus carrying a mixed field: `exclude_*` is **pure `must_not`**, so a row *missing* the field survives the exclusion; `unassigned` is **absence**, which no combination of excludes can express; and clearing an owner writes `""`, which OpenSearch counts as existing, so a bare `exists` would keep calling that row owned. Also here: `package_name`, the include+exclude **422**, the contributors and approvals **CSV exports** (header, sanitizer, and agreement with their JSON lens), and the **fleet-scoped audit rule** — a login journals with `cluster_id: null` yet must stay visible under a cluster-scoped read (the operator catch of 2026-07-18, and the backend half of the users panel's "Recent sign-ins" lens). Idempotent (decision revoked, SLA/name and the one test assignment restored in-phase). The **jobs/lease section** (#520) walks the *other* door onto the background jobs: the HTTP trigger (`404` unknown kind · `422` dry-run on a non-lifecycle kind · `200` inline lifecycle dry-run, asserted to have dropped nothing by diffing the index list) and the `system-jobs` lease itself — a live heartbeat gives `409`, a heartbeat silent past the TTL reads `stale: true` and is reclaimable (`202`), and the reclaimed run is waited out so the rig never exits holding a lease. It contains the rig's **one sanctioned direct store write** (seeding the lease doc: there is no API that parks a lease in a held state without actually running a job), marked as such in-place and superseded in-phase; the reclaim probe deliberately uses `staleness_sweep`, never `lifecycle_sweep`, because a reclaim *succeeds* and the job then really runs. Idempotent (decision revoked, SLA/name restored in-phase). The **client-events section** (#453/#520) covers the *browser* half of this same log stream: a valid beacon batch is `204` and lands in `backend.log` as `client.<name>`, and the probe deliberately forges `username`/`client_event` in its payload to prove the two safety properties by observation — the **namespace** (a client can never mint a line whose `event` collides with a real backend one) and **nesting, never splatting** (the server's own attribution survives at the top level while the forged keys stay contained under `fields`). CronJob/Helm packaging is deliberately NOT here — that's M10. |
| [`smoke-two-cluster.sh`](smoke-two-cluster.sh) | The **two-cluster isolation smoke** (issue #431): creates/reuses the small `beta` k3d tenant (`development/setup/seed-beta-workloads.yaml`, RUNNING-THE-STACK §B6), runs one cycle per scanner against it, then asserts the cross-bleed matrix — beta's namespace facet is exactly its seeded world, alpha's per-scanner counts don't move during beta ingest, no namespace leaks either direction, the **shared-digest trap** (payments reuses alpha's digests) serves per-tenant result sets (API total == that tenant's own store count), and the edge guards hold (missing `cluster_id` 422 · cross-tenant envelope 403 · cross-scanner token 403 · garbage token 401). Beta is left running; `TWOC_TEARDOWN=1` deletes the k3d cluster (its data stays — a quiet tenant, by design). Idempotent across re-runs. |
...
./development/e2e/smoke-two-cluster.sh                        # two-tenant isolation smoke (#431)
...
> check fails visibly — that's the point.
...
**Settings** (env): `LOAD_HEAVY=1` (default; `0` = gentle) · `LB_CLUSTERS`/`LB_DIGESTS`/`LB_FINDINGS`/

# M0b - Scanner image publish + compatibility/blessing CI
...
> self-built images **publishable and version-blessed** so operators can pin/swap a known-good tag (D41).

## Goal
Build and **publish** the pinned, self-built Trivy/Grype images (a small **blessed set** of versions), each
...
## Deliverables
- **Matrix build** (`docker buildx bake` + GHA matrix) of one Dockerfile per scanner across the blessed
  versions (`--build-arg TRIVY_VERSION`/`GRYPE_VERSION`) → tagged images `…-trivy:<ver>`, `…-grype:<ver>`.
- **Compatibility/blessing test** (CI): run each **candidate** real binary on a fixed image, feed output
  through the adapters, and assert the contracts — severity canonicalization (D16), EPSS/KEV capture,
  envelope shape, and **provenance present** (`scanner_version` + DB fields). **Green → publish as blessed.**
...
- **Blessed-versions config** — a small curated set (**current + 1-2 prior**, not 5), each with an explicit
...
- The matrix builds **both** images for every version in the blessed set; `docker build --check` clean.
...
- Blessed set + EOL policy documented; an EOL-DB-schema version is flagged (not silently shipped).

## Tests to write
- CI compat matrix: per blessed `(scanner, version)`, real-binary → adapters → contract asserts (incl. provenance).

...
- `backend/app/ingest/scan_order.py` + `POST /api/v1/scan-runs` — **backend-allocated** monotonic `scan_order` (**D45** — amends D40's source; never a clock): per-`(cluster_id, scanner)` counter doc CAS in `javv-scan-watermarks`, token-scoped, fail-closed scanner fetch at cycle start (replaces the scanner's `time.time_ns()` mint in `scanner/envelope.py new_scan_run`).

...
- `backend/app/api/search.py` — GET search/agg/trend/contributors endpoints; `extra="forbid"` request models; `cluster_id` via the one `tenant_search` chokepoint (SEC-4), entitlement on every fetch **and export** (IDOR).

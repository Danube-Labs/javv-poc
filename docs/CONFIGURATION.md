...
| `JAVV_EXPORT_TTL_HOURS` | `24` ⏳ | Scheduled reports (M7): global retention for a completed export — the result (stored in OpenSearch as chunks) is TTL-swept `JAVV_EXPORT_TTL_HOURS` after completion; a download past it → **410**. ⏳ *lands with [#32](https://github.com/Danube-Labs/javv-poc/issues/32) (M7)* | n/a (deploy) |
| `JAVV_EXPORT_MAX_BYTES` | `524288000` (500 MiB) ⏳ | Scheduled reports (M7): per-export hard size ceiling — the drain marks a job **failed** past it, so one job can't fill the store. ⏳ *lands with [#32](https://github.com/Danube-Labs/javv-poc/issues/32) (M7)* | n/a (deploy) |
| `JAVV_REPORT_DRAIN_SLEEP_MS` | `200` ⏳ | Scheduled reports (M7): off-peak throttle — the drain sleeps this long between export pages so a large run doesn't starve ingest (the PLAN gate). ⏳ *lands with [#32](https://github.com/Danube-Labs/javv-poc/issues/32) (M7)* | n/a (deploy) |
| `JAVV_REPORT_LEASE_TTL_SECONDS` | `300` ⏳ | Scheduled reports (M7): a claimed job's lease — a worker refreshes `heartbeat_at`; past `lease_expires_at` (no heartbeat) the next drain reclaims it (`retry_count`++). Match to the drain CronJob cadence. ⏳ *lands with [#32](https://github.com/Danube-Labs/javv-poc/issues/32) (M7)* | n/a (deploy) |

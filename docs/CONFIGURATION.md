...
| `JAVV_EXPORT_MAX_ROWS` | `50000` | Inline "run now" export hard row cap (audit A-M6/[#189](https://github.com/Danube-Labs/javv-poc/issues/189)), shared by every inline export: findings CSV + VEX, audit CSV, contributors CSV. On the swept exports a cheap pre-count runs before any PIT/stream — a lens over this → **413** (narrow the filters, or use M7's scheduled export). The contributors CSV ([#359](https://github.com/Danube-Labs/javv-poc/issues/359)) is an aggregation, not a sweep: its row count only exists after the query, so there the cap is a **post-count backstop** and is not reached in practice (the leaderboard is already bounded by its 100-actor board). Applies to the inline path only. | n/a (deploy) |

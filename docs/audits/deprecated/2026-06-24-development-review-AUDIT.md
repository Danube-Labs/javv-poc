...
- [x] **N2 — `pre-commit` hooks framework.** ✅ `.pre-commit-config.yaml` (ruff + conventional-commit) + installed by setup-dev.sh. `git-workflow.md` forbids `--no-verify` but no hooks exist.
...
- [x] **N8 — Status-tracking duplication:** ✅ live status moved to **GitHub issues + project board** (#22–#41, one per bolt, label `bolt`); each bolt README links its issue; PRs `Closes #N`. README table is dependency-map only. `bolts/README.md` table vs each bolt's `Status:` field will
